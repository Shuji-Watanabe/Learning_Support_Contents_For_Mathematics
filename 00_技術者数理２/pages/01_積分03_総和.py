...
f"""
$$
    \\displaystyle \\sum_{{k=1}}^{{n}} \\left\\{{ {ak_latex}  \\right \\}}  = {sum_ak_latex }
$$
"""
with st.expander("Latex コード"):
    st.write("\" \\sum_{k=1}^{n} \\left\\\\{", ak_latex ,"\\right\\\\} = ", sum_ak_latex,"\"")  


if st.checkbox("途中計算の表示") :
    disp_columns2_1 = st.columns([1,4])
    with disp_columns2_1[0]:
        tmp_radio = st.radio("表示形式",("和","因数分解","展開"),horizontal=True)
    ...
    with disp_columns2_1[1]:
        ...
        """
        with st.expander("Latex コード"):
            st.write("\"",ak_latex,"=",ak_sym_expand_latex,"\"")
        st.write("② 計算結果")
        f"""
        $$
        ...
        """
        with st.expander("Latex コード"):
            st.write("\"",disp_expr.replace('\\{','\\\\{').replace('\\}','\\\\}'),"\"")